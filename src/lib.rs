...
//!
//! Derived from VTE 0.15.0 for Vivido and related projects. This crate keeps the
//! standard-library parser and optional ANSI/Serde support. Unsupported OSC
//! diagnostics are formatted only when debug logging is enabled.
//!
...
/// [`Perform`]: trait.Perform.html
///
/// The const parameter preserves upstream parser type specialization. OSC data
/// uses a growable buffer; the parameter does not impose a size limit.
#[derive(Default)]
pub struct Parser<const OSC_RAW_BUF_SIZE: usize = 1024> {
    state: State,
    ...
    param: u16,
    osc_raw: Vec<u8>,
    /// End offset of each OSC parameter inside `osc_raw`.
    ///
    /// Parameters are stored back to back without their `;` separators, so the
    /// start of parameter `i` is the end of parameter `i - 1`, or `0` for the
    /// first one. Only the ends need to be tracked.
    osc_ends: [usize; MAX_OSC_PARAMS],
...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    ///
    /// Crate-internal: the synchronized-update machinery in [`crate::ansi`] is
    /// the only caller.
    #[cfg(feature = "ansi")]
    #[inline]
    ...
    fn action_osc_put_param(&mut self) {
        ...
        self.osc_ends[param_idx] = self.osc_raw.len();
        self.osc_num_params = param_idx + 1;
    }
    ...
    fn osc_dispatch<P: Perform>(&self, performer: &mut P, byte: u8) {
        ...
        let mut start = 0;
        for (slice, &end) in slices.iter_mut().zip(&self.osc_ends[..num_params]) {
            *slice = &self.osc_raw[start..end];
            start = end;
        }

        performer.osc_dispatch(&slices[..num_params], byte == 0x07);
    }
    ...
    fn advance_ground<P: Perform>(&mut self, performer: &mut P, bytes: &[u8]) -> usize {
        ...
        match str::from_utf8(&bytes[..plain_chars]) {
            ...
            Err(err) => {
                ...
                // SAFETY: `Utf8Error::valid_up_to` reports the length of a prefix that
    ...
    fn advance_partial_utf8<P: Perform>(&mut self, performer: &mut P, bytes: &[u8]) -> usize {
        ...
        match str::from_utf8(&self.partial_utf8[..self.partial_utf8_len]) {
            ...
            Ok(parsed) => {
                // SAFETY: This is only reached with `partial_utf8_len >= 1`, so `parsed`
                // is a non-empty `str` and always yields at least one character.
            ...
            Err(err) => {
                ...
                if valid_bytes > 0 {
                    // SAFETY: `Utf8Error::valid_up_to` reports the length of a prefix that
                    // is guaranteed to be valid UTF-8, and the `valid_bytes > 0` check
                    // above means that prefix is non-empty, so it yields a character.
                    let c = unsafe {
...
pub trait Perform {
    ...
    ///
    /// The crate-internal `Parser::advance_until_terminated` consults this to
    /// stop the parser after receiving certain escape sequences like
